...
struct CollectiveMainloop {
  ...
  template <typename SharedStorage, typename FrgTensorC,
            typename TranscriptTensor>
  CUTLASS_DEVICE void mma_producer_consumer(
      Params const& mainloop_params, MainloopPipeline pipeline,
      PipelineState& smem_pipe_read, PipelineState& smem_pipe_write,
      FrgTensorC& tCrC, TranscriptTensor& transcript_extraction_tensor,
      int thread_idx, SharedStorage& shared_storage,
      cute::tuple<int32_t, int32_t, int32_t> block_coord, int k_tile_count,
      uint16_t const tma_mcast_mask_a, uint16_t const tma_mcast_mask_b,
      bool is_tma_loader_warp) {
    ...
    int const tma_prologue = min(kStages, k_tile_count);
    CUTLASS_PRAGMA_UNROLL
    for (int k_tile = 0; k_tile < tma_prologue; ++k_tile) {
      if (is_tma_loader_warp) {
        int lane_predicate = cute::elect_one_sync();
        if (lane_predicate) {
          pipeline.producer_acquire(smem_pipe_write);
          BarrierType* tmaBar = pipeline.producer_get_barrier(smem_pipe_write);
    ...
    for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
      ...
      int const next_k_tile = k_tile + kStages;
      if (next_k_tile < k_tile_count && is_tma_loader_warp) {
        int lane_predicate = cute::elect_one_sync();
        if (lane_predicate) {
          pipeline.producer_acquire(smem_pipe_write);
          BarrierType* tmaBar = pipeline.producer_get_barrier(smem_pipe_write);
          ...
          copy(mainloop_params.tma_load_A.with(*tmaBar, tma_mcast_mask_a),
               tAgA(_, next_k_tile), tAsA(_, write_stage));
          copy(mainloop_params.tma_load_B.with(*tmaBar, tma_mcast_mask_b),
               tBgB(_, next_k_tile), tBsB(_, write_stage));
          pipeline.producer_commit(smem_pipe_write, TmaTransactionBytes);
          ++smem_pipe_write;
        }
      }
