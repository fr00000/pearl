...
struct ClusterPersistentTileScheduler {

 public:
  // Host side kernel arguments
  struct Arguments {
    int const num_blocks_m, num_blocks_n, num_clusters_m, num_clusters_n,
        cluster_size_m, cluster_size_n, launched_clusters, swizzle;
    bool const swizzle_n_maj;
  };

  // Device side kernel params
  struct Params {
    SwizzleParams const swizzle_params;
    int const total_clusters;
    int const launched_clusters;
    int const launch_grid_clusters_m;
  };

  static Params to_underlying_arguments(Arguments const& args) {
    SwizzleParams swizzle_params = make_swizzle_params<Arguments>(args);
    int const total_clusters = args.num_clusters_m * args.num_clusters_n;
    return {
        .swizzle_params = swizzle_params,
        .total_clusters = total_clusters,
        .launched_clusters = args.launched_clusters,
        .launch_grid_clusters_m = args.launched_clusters,
    };
  }

  static dim3 get_grid_dim(Arguments const& args, int num_sm) {
    return {uint32_t(args.launched_clusters * args.cluster_size_m),
            uint32_t(args.cluster_size_n), 1};
  }
  ...
  struct WorkTileInfo {
    ...
    bool is_valid(Params const& params) const {
      return is_valid_tile && linear_cluster_idx < params.total_clusters;
    }
    ...
    CUTLASS_DEVICE cute::tuple<int32_t, int32_t, int32_t> get_block_coord(
      ...
      auto [ctaid_in_cluster_x, ctaid_in_cluster_y, ctaid_in_cluster_z] =
          block_id_in_cluster();
      auto [nonmaj_cluster, maj_cluster] =
          get_coords_from_linear_idx(params.swizzle_params, linear_cluster_idx);
      int m_cluster =
          params.swizzle_params.swizzle_n_maj ? nonmaj_cluster : maj_cluster;
      int n_cluster =
          params.swizzle_params.swizzle_n_maj ? maj_cluster : nonmaj_cluster;
      int m_block = m_cluster * size<0>(ClusterShape{}) + ctaid_in_cluster_x;
      int n_block = n_cluster * size<1>(ClusterShape{}) + ctaid_in_cluster_y;
      return {m_block, n_block, 1};
    }
  ...
  CUTLASS_DEVICE
  ClusterPersistentTileScheduler() {}

  CUTLASS_DEVICE
  WorkTileInfo get_initial_work(Params const& params) const {
    auto [cidx, cidy, cidz] = cluster_id_in_grid();
    int const linear_cluster_idx =
        cidx + cidy * params.launch_grid_clusters_m;
    return {linear_cluster_idx, linear_cluster_idx < params.total_clusters};
  }
  ...
  CUTLASS_DEVICE WorkTileInfo
  get_next_work(Params const& params, WorkTileInfo const& current_work) const {
    int const next_cluster_idx =
        current_work.linear_cluster_idx + params.launched_clusters;
    return {next_cluster_idx, next_cluster_idx < params.total_clusters};
  }
};
